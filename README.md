...
7. Installs the [`dev.newedia.t3-awake`](docs/t3-awake.md) LaunchAgent, which keeps the Mac awake while a T3 Code thread is working.
...
`bin/agents_link all` projects that core into each installed harness. It uses native global rule files for Claude, Codex, and OpenCode; installs each shared skill additively without replacing harness-native skills; and generates Cursor's rule and hook files.

Cursor is the exception that needs explaining. It has no global rules file — it finds rules by walking up from the workspace directory, reading `.cursor/rules/**/*.mdc` and `AGENTS.md` at every ancestor. So `agents_link` generates `~/.cursor/rules/agents.mdc` with `alwaysApply: true`, which Cursor reaches because `$HOME` is an ancestor of anything below it. **That covers every project under your home directory and nothing outside it** — a repo at `/opt/work/thing` gets no shared rules, silently. The conformance report states that limit rather than claiming global coverage. Cursor also cannot take shared subagents: its CLI reads them only from a workspace `.cursor/agents`, never from `~/.cursor/agents`, so projecting there would fail with no error at all.

Rules and skills are the same bytes everywhere, so they are symlinked — a skill written into `~/.claude/skills/` physically lands in `agents/skills/` through the link chain and is instantly shared with every other harness as a reviewable diff. `~/.agents/` is the cross-tool convention: Codex and OpenCode scan `~/.agents/skills` natively, `~/.agents/AGENTS.md` links to the same shared rules, and `npx skills --global` installs there. Cursor resolves skills from an explicit user-scope list — `~/.cursor/skills`, `~/.agents/skills`, `~/.claude/skills`, `~/.codex/skills` — independent of project location, so it is the one dimension where Cursor has no caveat at all.

Subagents cannot be symlinked, because Claude and OpenCode disagree about the frontmatter. `bin/agents_render` translates each canonical file instead, and `agents_link` writes the result to `claude/agents/` and `opencode/agents/` — both of which are live harness directories in this working tree. Those generated files are untracked and carry a `generated by bin/agents_link` header; edit `agents/subagents/<name>.md` and re-run `bin/agents_link all`. Codex has no stable subagent format and Cursor's is unverified, so both are reported as not applicable rather than guessed at. Model tiers are dropped on OpenCode (its model ids are machine-local) — the full translation contract and its lossy cases are documented alongside the sources.
...
`~/.claude` is a symlink to this repository's `claude/`. That inversion is deliberate. Claude Code rewrites its own `settings.json` — it once dropped every hook, permission, and statusline entry this repo had configured while adding six plugins of its own — so a tracked file that is merely *copied outward* rots silently and the live file always wins. Pointing Claude at the working tree makes the live file the tracked file: anything it rewrites shows up in `git diff`, which is the only thing that keeps it honest.

Credentials still never enter the repository. `claude/settings.json` carries an `apiKeyHelper` command rather than a token; `claude/api-key-helper.sh` is tracked and reads `claude/auth-token`, which is gitignored and mode `600`. The base URL is a localhost proxy and the model ids are not secrets, so those stay tracked in `env`. `claude_link --check` fails hard if a literal `ANTHROPIC_AUTH_TOKEN` ever appears in the tracked settings.

Everything Claude writes at runtime — `sessions/`, `projects/`, `history.jsonl`, `shell-snapshots/`, `plugins/`, `security/`, caches — lands in the working tree and is gitignored, so `git status` stays clean while ~340 MB of state sits beside the five tracked files. `~/.claude.json` is **not** part of this: it stays at the home root, holding OAuth and per-project state.
...
`--migrate` clones `~/.claude` to `~/.claude.pre-migrate-backup` (an APFS clone, so it is near-instant and nearly free), moves everything the repo does not already track into `claude/`, drops its own stale per-file links, and lets the repo copy win every collision. Remove the backup once you are satisfied.

`CLAUDE_CONFIG_DIR` would also relocate the directory and is the officially documented mechanism, but it depends on the environment being set — any launch that does not inherit it silently falls back to a fresh `~/.claude`. The symlink has no such failure mode. (Note that on this build `CLAUDE_CONFIG_DIR` also relocates `~/.claude.json`, which the published docs say it does not.)

Authentication, provider settings, sessions, transcripts, caches, and model selection remain native and machine-local for every harness. The bootstrap still installs Claude Code with Anthropic's native self-updating installer when `claude` is missing.
...
| `~/.claude.json` | Stays at the home root: OAuth, per-project state, and user MCP configuration |
