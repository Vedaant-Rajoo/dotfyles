...
function assert_function --argument-names function_name description
...
function assert_no_mock_calls
    if test (count $u_test_mock_calls) -eq 0
        harness_pass 'source runs no top-level commands in test mode'
    else
        harness_fail 'source runs top-level commands in test mode' \
...
assert_function u_main 'source exposes u_main'
assert_function u_outdated_names 'source exposes u_outdated_names'
assert_function u_cask_app_paths 'source exposes u_cask_app_paths'
assert_function u_inspect_app 'source exposes u_inspect_app'
assert_function u_prepare_cask 'source exposes u_prepare_cask'
assert_function u_confirm_running_apps 'source exposes u_confirm_running_apps'
assert_function u_stop_app 'source exposes u_stop_app'
assert_function u_stop_cask_apps 'source exposes u_stop_cask_apps'
assert_function u_recheck_cask_apps 'source exposes u_recheck_cask_apps'
assert_function u_upgrade_cask 'source exposes u_upgrade_cask'
assert_function u_reopen_closed 'source exposes u_reopen_closed'
assert_no_mock_calls

set -l literal_log (mktemp)
u_log "$literal_log" 'literal\ntext'
assert_equal 'literal\ntext' "$(string collect <"$literal_log")" \
    'log messages preserve literal backslash escapes'
rm -f "$literal_log"

# u_outdated_names

assert_equal '' "$(u_outdated_names formulae '{"formulae":[],"casks":[]}')" \
    'outdated names emits nothing for an empty section'

assert_equal jq "$(u_outdated_names formulae '{"formulae":[{"name":"jq"}],"casks":[{"name":"cursor"}]}')" \
    'outdated names emits the outdated formula name'

assert_equal cursor "$(u_outdated_names casks '{"formulae":[{"name":"jq"}],"casks":[{"name":"cursor"}]}')" \
    'outdated names emits the outdated cask name'

assert_equal 'jq
fish' "$(u_outdated_names formulae '{"formulae":[{"name":"jq"},{"name":"fish"}],"casks":[]}')" \
    'outdated names emits one name per line'

u_outdated_names formulae 'not json' >/dev/null 2>&1
assert_equal 5 $status 'outdated names rejects invalid JSON'

u_outdated_names formulae '{"formulae":[{"installed_versions":["1.0"]}],"casks":[]}' >/dev/null 2>&1
assert_equal 1 $status 'outdated names rejects a record without a name'

u_outdated_names formulae '{"formulae":{},"casks":[]}' >/dev/null 2>&1
assert_equal 1 $status 'outdated names rejects a non-array section'
...
assert_equal '' "$(u_outdated_names casks '{"formulae":[]}')" \
    'outdated names treats a missing section as empty'
...
assert_equal 12 (count $pretty_outdated) 'pretty outdated fixture splits into several arguments'
assert_equal jq "$(u_outdated_names formulae $pretty_outdated)" \
    'outdated names accepts multiline pretty-printed JSON'
...
assert_equal /Applications/Cursor.app "$(u_cask_app_paths cursor $batched_json)" \
    'cask app paths emits only the app artifact target'

u_cask_app_paths font-hack $batched_json >/dev/null
assert_equal 0 $status 'cask app paths accepts a cask without app artifacts'
assert_equal '' "$(u_cask_app_paths font-hack $batched_json)" \
    'cask app paths emits nothing for a cask without app artifacts'

assert_equal '' "$(u_cask_app_paths ghost $batched_json)" \
    'cask app paths emits nothing for a token the document does not describe'
...
/Applications/Two.app' "$(u_cask_app_paths multi $cask_two_apps_json)" \
    'cask app paths emits every app artifact target'
...
assert_equal 1 $status 'cask app paths rejects a missing app target'
...
assert_equal 1 $status 'cask app paths rejects a non-absolute app target'

u_cask_app_paths bad 'not json' >/dev/null 2>&1
assert_equal 5 $status 'cask app paths rejects invalid JSON'
...
assert_equal '/Applications/Google Chrome.app' "$(u_cask_app_paths google-chrome $cask_spaced_json)" \
    'cask app paths preserves an app target containing spaces'
...
assert_equal '/Applications/Google Chrome.app' "$(u_cask_app_paths google-chrome $cask_spaced_pretty)" \
    'cask app paths handles spaces in multiline JSON'
...
assert_equal 1 $status 'cask app paths rejects a late malformed artifact'
...
assert_equal 1 (count $U_SKIPS) 'record skip keeps one row per token'
assert_equal 'cursor|first reason' "$(string replace -a \t '|' -- $U_SKIPS[1])" \
    'record skip keeps the first recorded reason'
u_token_skipped cursor
assert_equal 0 $status 'token skipped finds a recorded token'
u_token_skipped calm
assert_equal 1 $status 'token skipped misses an unrecorded token'
...
assert_equal 'cask-upgrade|cursor|brew upgrade --cask failed with status 9' \
    ...
    'record failure keeps category, subject, and detail'
u_token_has_failure cursor
assert_equal 0 $status 'token has failure finds a recorded subject'
u_token_has_failure calm
assert_equal 1 $status 'token has failure misses an unrecorded subject'

set -g U_CLOSED
u_record_closed cursor /Applications/Cursor.app
u_record_closed cursor /Applications/Cursor.app
assert_equal 1 (count $U_CLOSED) 'record closed keeps one row per app'
...
assert_equal 0 $status 'reopen closed succeeds for one token'
assert_equal /Applications/Cursor.app "$u_test_open_calls[1]" \
    'reopen closed opens the requested token app'
assert_equal 1 (count $u_test_open_calls) 'reopen closed leaves other tokens closed'
assert_equal 1 (count $U_CLOSED) 'reopen closed consumes only the requested token rows'

u_reopen_closed
assert_equal 0 $status 'reopen closed sweeps every remaining row'
assert_equal /Applications/Calm.app "$u_test_open_calls[2]" \
    'reopen closed opens the remaining app in the sweep'
assert_equal 0 (count $U_CLOSED) 'reopen closed empties the closed list'
...
u_reopen_closed
assert_equal 1 $status 'reopen closed reports a failed open'
assert_equal 1 (count $U_CLOSED) 'a failed open keeps its row for a later retry'
assert_equal 'app-reopen|cursor|unable to reopen /Applications/Cursor.app' \
    "$(string replace -a \t '|' -- $U_FAILURES[1])" \
    'a failed open records an app-reopen failure'

set -g u_test_open_fail
u_reopen_closed
assert_equal 0 $status 'a later pass retries a previously failed open'
assert_equal 0 (count $U_CLOSED) 'the retry consumes the recovered row'
...
assert_equal 0 $status 'confirm approves closing running applications'
assert_equal 1 $u_test_gum_calls 'confirm asks exactly once'
assert_equal 0 (count $U_SKIPS) 'an approved confirmation records no skips'
assert_equal 1 (count (string match -a -- '*Cursor (cursor)*' $u_test_confirm_output)) \
    'confirm lists the running application with its token'
assert_equal 0 (count (string match -a -- '*Calm*' $u_test_confirm_output)) \
    'confirm never lists an application that is not running'

set -g u_test_gum_status 1
set -g U_SKIPS
u_confirm_running_apps >/dev/null
assert_equal 2 $status 'confirm reports a deliberate decline'
assert_equal 'cursor|declined closing running applications' \
    "$(string replace -a \t '|' -- $U_SKIPS[1])" \
    'a decline skips the running token'
assert_equal 1 (count $U_SKIPS) 'a decline never skips a token that is not running'

set -g U_APPS "calm"\t"/Applications/Calm.app"\t"com.calm.Calm"\t""
set -g U_SKIPS
set -g u_test_gum_calls 0
u_confirm_running_apps >/dev/null
assert_equal 0 $status 'confirm passes when nothing is running'
assert_equal 0 $u_test_gum_calls 'confirm never prompts when nothing is running'
...
assert_equal 0 $status 'stop app succeeds when the app closes'
assert_equal 'terminate com.cursor 111' "$u_test_lifecycle_calls[1]" \
    'stop app terminates the exact original PIDs'
assert_equal 'running com.cursor 111' "$u_test_lifecycle_calls[2]" \
    'stop app reconciles against the exact original PIDs'
assert_equal 'cursor|/Applications/Cursor.app' "$(string replace -a \t '|' -- $U_CLOSED[1])" \
    'stop app records the closed application'

set -g u_test_lifecycle_calls
set -g U_CLOSED
set -g u_test_lifecycle_script 111
u_stop_app terminate cursor /Applications/Cursor.app com.cursor 111 >/dev/null
assert_equal 2 $status 'stop app reports an app that survives every poll'
assert_equal 11 (count $u_test_lifecycle_calls) 'stop app polls ten times before giving up'
assert_equal 0 (count $U_CLOSED) 'a surviving app is never recorded as closed'

set -g U_FAILURES
set -g U_SKIPS
set -g u_test_lifecycle_running_status 1
u_stop_app terminate cursor /Applications/Cursor.app com.cursor 111 >/dev/null
assert_equal 1 $status 'stop app fails when reconciliation cannot be trusted'
assert_equal 1 (count (string match -a -- 'app-running*' $U_FAILURES)) \
    'an unreliable reconciliation records a failure'
assert_equal 1 (count $U_SKIPS) 'an unreliable reconciliation skips the cask'
set -g u_test_lifecycle_running_status 0
...
set -g u_test_lifecycle_script -
u_stop_app terminate cursor /Applications/Cursor.app com.cursor 111 >/dev/null
assert_equal 0 $status 'stop app still reconciles after a helper failure'
assert_equal 1 (count (string match -a -- 'app-terminate*' $U_FAILURES)) \
    'a helper failure is recorded even when the app closed'
set -g u_test_lifecycle_op_status 0
...
assert_equal 2 $status 'recheck skips when an application instance appeared'
assert_equal 'cursor|an application instance appeared before upgrade: /Applications/Cursor.app' \
    "$(string replace -a \t '|' -- $U_SKIPS[1])" \
    'recheck names the app that reappeared'
...
set -g U_SKIPS
u_recheck_cask_apps cursor
assert_equal 0 $status 'recheck passes when no instance remains'
...
set -g U_FAILURES
u_recheck_cask_apps cursor
assert_equal 1 $status 'recheck fails when it cannot be trusted'
assert_equal 1 (count (string match -a -- 'app-inspect*' $U_FAILURES)) \
    'an untrustworthy recheck records a failure'
...
set -g u_mock_root (harness_tmpdir)
...
# Write an executable PATH mock. Every mock first logs its invocation to
# U_MOCK_LOG so assertions can pin call shapes and order.
function u_test_mock --argument-names name body
    printf '#!/usr/bin/env fish\necho "%s $argv" >>"$U_MOCK_LOG"\n%s\n' $name $body >"$u_mock_bin/$name"
    chmod +x "$u_mock_bin/$name"
end
...
u_test_mock brew 'if test "$argv[1]" = info; and test "$argv[2]" = --cask; and test "$argv[3]" = --json=v2; and test (count $argv) -ge 4
...
u_test_mock osascript 'if test "$argv[1]" != -l; or test "$argv[2]" != JavaScript; or not string match -q -- "*/u-cask-lifecycle.js" "$argv[3]"
...
u_test_mock gum 'if test "$argv[1]" = confirm
...
u_test_mock open 'if test -e "$U_MOCK_DIR/open-always-fail"
    exit 1
end
exit 0'
...
function assert_call_before --argument-names earlier_pattern later_pattern description
    ...
    if test -n "$earlier_line"; and test -n "$later_line"; and test "$earlier_line" -lt "$later_line"
        ...
        harness_fail "$description" \
            'earlier line: '(string escape -- "$earlier_line") \
            'later line:   '(string escape -- "$later_line")
    end
end
...
assert_equal '/Applications/Cursor.app|com.todesktop.230313mzl4w4u92|111' \
    ...
    'inspect app emits path, bundle identifier, and PIDs'

assert_equal '/Applications/Calm.app|com.calm.Calm|' \
    "$(u_inspect_app /Applications/Calm.app | string replace -a \t '|')" \
    'inspect app emits an empty PID column for a stopped app'

u_inspect_app /Applications/Missing.app >/dev/null 2>&1
assert_equal 1 $status 'inspect app propagates a helper failure'

u_inspect_app >/dev/null 2>&1
assert_equal 1 $status 'inspect app rejects an empty bundle path'
...
assert_equal 0 $u_test_main_status 'main succeeds when no packages are outdated'
assert_equal 1 (u_test_calls '^nvim ') \
    'main performs all Neovim maintenance in one invocation'
assert_equal 1 (u_test_calls '^nvim --headless +Lazy! sync +MasonUpdate +TSUpdate +qa$') \
    'the Neovim invocation carries every maintenance command'
assert_equal 1 (u_test_calls '^brew update$') \
    'main end-to-end cases use the Homebrew update mock'
assert_equal 1 (u_test_calls '^brew outdated --json=v2$') \
    'main discovers outdated packages with one JSON call'
assert_equal 0 (u_test_calls '^brew upgrade ') \
    'main runs no upgrade when the outdated document is empty'
...
assert_equal 1 $u_test_main_status 'declining the initial confirmation aborts the run'
assert_equal 0 (u_test_calls '^nvim ') \
    'declining the initial confirmation stops before maintenance'

u_test_reset
printf '%s\n' '{"formulae":[],"casks":[]}' >"$U_MOCK_DIR/outdated.json"
printf '6\n' >"$U_MOCK_DIR/exit-brew-outdated"
u_test_run_main outdated-failure
assert_equal 6 $u_test_main_status 'outdated discovery preserves the Homebrew failure status'
assert_equal 0 (u_test_calls '^brew upgrade ') \
    'failed outdated discovery stops before package upgrades'
assert_equal 0 (u_test_calls '^brew doctor$') \
    'failed outdated discovery stops before Homebrew doctor'

u_test_reset
printf '%s\n' '{"formulae":[],"casks":[]}' >"$U_MOCK_DIR/outdated.json"
printf '4\n' >"$U_MOCK_DIR/exit-brew-doctor"
u_test_run_main doctor-failure
assert_equal 1 $u_test_main_status 'Homebrew doctor failure produces an aggregate failure'
assert_equal 1 (u_test_output_contains 'all [brew-doctor]: brew doctor failed') \
    'main surfaces a Homebrew doctor failure in final details'

u_test_reset
printf '%s\n' '{"formulae":[{"name":"jq"},{"name":"fish"}],"casks":[]}' >"$U_MOCK_DIR/outdated.json"
u_test_run_main formula-only
assert_equal 0 $u_test_main_status 'main succeeds after a formula-only upgrade'
assert_equal 1 (u_test_calls '^brew upgrade --formula jq fish$') \
    'main upgrades named formulae in one formula-only command'
assert_equal 0 (u_test_calls '^brew upgrade --cask ') \
    'formula-only updates never invoke a cask upgrade'
...
assert_equal 7 $u_test_main_status 'formula upgrade failure is fail-fast'
assert_equal 0 (u_test_calls '^brew info --cask ') \
    'formula failure stops before cask preparation'
assert_equal 0 (u_test_calls '^brew doctor$') \
    'formula failure stops before Homebrew doctor'

u_test_reset
printf '%s\n' '{"formulae":[],"casks":[{"name":"font-hack"}]}' >"$U_MOCK_DIR/outdated.json"
u_test_write_cask_info '{"casks":['$u_cask_font_json']}'
u_test_run_main non-app-cask
assert_equal 0 $u_test_main_status 'main upgrades a non-app cask'
assert_equal 1 (u_test_calls '^brew upgrade --cask font-hack$') \
    'a non-app cask upgrades in its own cask-only command'
assert_equal 0 (u_test_calls '^osascript ') \
    'a non-app cask uses no application lifecycle operations'
...
assert_equal 1 $u_test_main_status 'Homebrew cleanup failure produces an aggregate failure'
assert_equal 1 (u_test_calls '^brew doctor$') \
    'main still doctors Homebrew after cleanup fails'
assert_equal 1 (u_test_output_contains 'all [brew-cleanup]: brew cleanup --prune=all failed') \
    'main surfaces a Homebrew cleanup failure in final details'

u_test_reset
printf '%s\n' '{"formulae":[],"casks":[{"name":"cursor"},{"name":"calm"}]}' >"$U_MOCK_DIR/outdated.json"
printf '3\n' >"$U_MOCK_DIR/exit-brew-info"
u_test_run_main info-failure
assert_equal 1 $u_test_main_status 'a failed batched metadata call fails the run'
assert_equal 0 (u_test_calls '^brew upgrade --cask ') \
    'no cask upgrades without trustworthy metadata'
assert_equal 1 (u_test_output_contains 'Failed casks (2): cursor, calm') \
    'a failed metadata call fails every outdated cask'
...
assert_equal 1 $u_test_main_status 'cask preparation failure produces a final failure'
assert_equal 0 (u_test_calls '^brew upgrade --cask bad-target$') \
    'main never upgrades a cask with unreliable app metadata'
assert_equal 1 (u_test_calls '^brew upgrade --cask calm$') \
    'main continues to a later cask after preparation fails'
assert_equal 1 (u_test_output_contains 'Failed casks (1): bad-target') \
    'main names a preparation failure in the failed-cask group'
...
assert_equal 1 $u_test_main_status 'one cask failure produces final nonzero status'
assert_equal 1 (u_test_calls '^brew upgrade --cask cursor$') \
    'main attempts the failing cask once'
assert_equal 1 (u_test_calls '^brew upgrade --cask calm$') \
    'main continues to a later cask after one cask fails'
assert_equal 1 (u_test_calls '^open /Applications/Cursor.app$') \
    'main reopens the failed cask application'
assert_equal 1 (u_test_calls '^brew cleanup --prune=all$') \
    'main cleans up Homebrew after a cask-level failure'
assert_equal 1 (u_test_calls '^brew doctor$') \
    'main doctors Homebrew after a cask-level failure'
assert_equal 1 (u_test_output_contains 'Cask summary: 1 upgraded, 0 skipped, 1 failed') \
    'main prints failed casks in aggregate counts'
assert_equal 1 (u_test_output_contains 'Failed casks (1): cursor') \
    'main names failed casks separately'
assert_equal 1 (u_test_output_contains 'brew upgrade phase completed with failures') \
    'main never prints an unconditional success message after a cask failure'
...
assert_equal 0 $u_test_main_status 'a deliberate lifecycle decline is not a command failure'
assert_equal 0 (u_test_calls '^brew upgrade --cask cursor$') \
    'main does not upgrade a deliberately skipped running-app cask'
assert_equal 1 (u_test_calls '^brew upgrade --cask font-hack$') \
    'main still upgrades a non-running cask after a deliberate skip'
assert_equal 1 (u_test_output_contains 'Skipped casks (1): cursor') \
    'main reports deliberately skipped casks separately'
assert_equal 1 (u_test_output_contains 'Cask summary: 1 upgraded, 1 skipped, 0 failed') \
    'main keeps skip and failure counts separate'
assert_equal 1 (u_test_output_contains 'brew upgrade phase completed with skipped casks') \
    'main identifies a phase that completed with deliberate skips'
...
assert_equal 0 $u_test_main_status 'declining force termination remains a deliberate skip'
assert_equal 0 (u_test_calls '^brew upgrade --cask cursor$') \
    'main never upgrades a cask whose force termination was declined'
assert_equal 0 (u_test_calls '^open ') \
    'an app that never closed is never reopened'
assert_equal 1 (u_test_output_contains 'Skipped casks (1): cursor') \
    'main classifies a declined force termination as skipped'
assert_equal 1 (u_test_output_contains 'Failed casks (0): none') \
    'main does not misclassify a declined force termination as failed'
...
assert_equal 1 $u_test_main_status 'an unsuccessful approved force termination fails the run'
assert_equal 0 (u_test_calls '^brew upgrade --cask cursor$') \
    'main never upgrades a cask whose application could not be stopped'
assert_equal 1 (u_test_output_contains 'Failed casks (1): cursor') \
    'main classifies an unsuccessful force termination as failed'
...
assert_equal 0 $u_test_main_status 'an instance appearing before upgrade is a deliberate skip'
assert_equal 0 (u_test_calls '^brew upgrade --cask cursor$') \
    'main never upgrades under an application instance that appeared'
assert_equal 1 (u_test_calls '^open /Applications/Cursor.app$') \
    'main reopens the app it closed when the recheck skips'
assert_equal 1 (u_test_output_contains 'an application instance appeared before upgrade') \
    'main names the recheck skip reason'
assert_equal 1 (u_test_output_contains 'Skipped casks (1): cursor') \
    'main classifies a recheck skip as skipped'
...
assert_equal 1 $u_test_main_status 'a failed reopen produces final nonzero status'
assert_equal 1 (u_test_calls '^brew upgrade --cask cursor$') \
    'the upgrade itself still ran before the failed reopen'
assert_equal 3 (u_test_calls '^open /Applications/Cursor.app$') \
    'the per-cask reopen, final sweep, and exit handler each retry the open'
assert_equal 1 (u_test_output_contains 'unable to reopen /Applications/Cursor.app') \
    'main surfaces the reopen failure in final details'
...
assert_equal 0 $u_test_main_status 'a tee failure does not replace successful brew pipeline status'
assert_equal 1 (u_test_output_contains 'Cask summary: 1 upgraded, 0 skipped, 0 failed') \
    'main counts the cask upgrade from brew status rather than tee status'
